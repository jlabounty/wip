...
//      v_h_esum.at( i )->Rebin( 256 ); //Digitized Rebinning...128...256...600
  ...
  for ( unsigned i = 0; i < v_h_esum_2.size(); i++ )
    {
//      v_h_esum_2.at( i )->Rebin( 600 ); //Digitized Rebinning
...
//  c_eemc_lin->Print("Plots/NewGeometry_Linearity.png");
  ...
  //eemc_lin_2->SetTitle("Response of Digitized Detector");
  eemc_lin_2->SetTitle("Response of Solid Cone [Raw]");
...
//  c_mg->Print("Plots/Comparison_Linearity.png");
  ...
  eemc_res->SetTitle("Resolution of New Geometry [Raw]");
...
//  TF1 *res_fit = new TF1("res_fit", "[0]*1/sqrt(x)", 0, 40);
  TF1 *res_fit = new TF1("res_fit", "sqrt(([0]*1/sqrt(x))^2 + (0.005)^2)", 0, 40);
...
//  c_eemc_res->Print("Plots/NewGeometry_Resolution.eps");
  ...
  eemc_res_2->SetTitle("Resolution of Solid Cone [Raw]");
...
//  TF1 *res_fit_2 = new TF1("res_fit_2", "sqrt(([0]*1/sqrt(x))^2 + (0.005)^2)", 0, 40);
