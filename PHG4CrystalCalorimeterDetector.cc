...
PHG4CrystalCalorimeterDetector::FillCrystalUnit(G4LogicalVolume *crystal_logic)
{
	...
	G4double dx_front_small = _dx_front / 4.0;	
	G4double dy_front_small = _dy_front / 4.0;	
	G4double dx_back_small = _dx_back / 4.0;	
	G4double dy_back_small = _dy_back / 4.0;
	...
	Crystal_Center = G4ThreeVector(0*mm, 0*mm, 0*mm);
	
	G4RotationMatrix *Rot = new G4RotationMatrix(); //rotation matrix for the placement of each crystal
	Rot->rotateX(0*rad);
	Rot->rotateY(0*rad);
	Rot->rotateZ((TMath::Pi() / 2)*rad);
